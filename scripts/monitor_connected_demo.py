...
Run this in a separate terminal while the guarded DEMO bot is trading. The
monitor performs no broker write and never changes production policy. It samples
the existing connected certification collector, writes local runtime evidence,
and maintains an aggregate summary for one account/symbol scope.
"""
...
def main() -> int:
    ...
    run_dir = args.output_dir / f"connected-demo-watch-{run_stamp}"
    ...
    for index in range(1, args.max_samples + 1):
        try:
            ...
            sample_path = run_dir / f"sample-{index:04d}.json"
            _atomic_json(sample_path, report)
            ...
            break
        except Exception as exc:  # evidence collection must not disturb the trading process
            event = {
