...
def test_profile_boundaries_are_preserved():assert resolve_profile(100) is RiskProfile.SMALL;assert resolve_profile(300) is RiskProfile.MEDIUM;assert resolve_profile(999.99) is RiskProfile.MEDIUM;assert resolve_profile(1000) is RiskProfile.NORMAL
...
def test_aggressive_aggregate_risk_ceiling_is_16_percent():
    p=TradePlan("p","o",Direction.BUY,100,99.9,101,None,"M5",10,"READY",());a=AccountFacts(1,"demo","USD",100,100,100,True,True);s=SymbolSpec("XAUUSDm",3,.001,.001,1,.01,200,.01);r=evaluate(p,a,s,day_start_equity=100,target_risk_pct=3.0,aggressive_mode=True,aggregate_open_risk_pct=15.0);assert r.decision is RiskDecision.BLOCK
