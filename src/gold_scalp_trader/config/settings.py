...
def _family_env() -> StrategyFamily | None:
    ...
    try:
        ...
        raise ValueError("ACTIVE_STRATEGY_FAMILY is invalid") from exc
...
def load_settings() -> Settings:
    ...
    aliases = tuple(
        dict.fromkeys(
            value.strip()
            for value in os.getenv("SYMBOL_ALIASES", "XAUUSDm,XAUUSD").split(",")
            if value.strip()
        )
    )
    ...
    settings = Settings(
        ...
        target_risk_percent=None if not target_raw else float(target_raw),
...
def validate_settings(settings: Settings) -> None:
    ...
    if settings.consecutive_loss_cooldown_minutes < 30:
        raise ValueError("loss cooldown cannot be below 30 minutes")
    ...
    if settings.mode is RuntimeMode.DEMO and settings.target_risk_percent is None:
        raise ValueError("TARGET_RISK_PERCENT is required for DEMO")
    ...
    if not settings.state_db_path:
        raise ValueError("STATE_DB_PATH cannot be empty")
    if settings.loop_interval_seconds < 0.5:
