...
def execute_once(*,store:StateStore,intent:ExecutionIntent,gate:GateDecision,lease:ControllerLease,writer:Mt5Writer,precheck_passed:bool)->ExecutionIntent:
    ...
    save(store,intent)
    if gate.state is not GateState.ALLOW:out=intent.with_state(IntentState.FAILED,reason="GATE_BLOCKED");save(store,out);return out
    if not verify(store,lease):out=intent.with_state(IntentState.FAILED,reason="CONTROLLER_FENCED");save(store,out);return out
    if not precheck_passed:out=intent.with_state(IntentState.FAILED,reason="PRECHECK_FAILED");save(store,out);return out
