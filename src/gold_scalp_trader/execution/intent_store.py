...
def save(store:StateStore,intent:ExecutionIntent)->None:store.put(NS,intent.intent_id,{"intent_id":intent.intent_id,"action":intent.action.value,"symbol":intent.symbol,"direction":intent.direction.value,"volume":intent.volume,"price":intent.price,"sl":intent.sl,"tp":intent.tp,"state":intent.state.value,"created_at":intent.created_at.isoformat(),"send_count":intent.send_count,"broker_ticket":intent.broker_ticket,"position_ticket":intent.position_ticket,"reason":intent.reason})
def load(store:StateStore,intent_id:str)->ExecutionIntent|None:
    r=store.get(NS,intent_id)
    if r is None:return None
    p=r.payload; return ExecutionIntent(str(p["intent_id"]),ExecutionAction(p["action"]),str(p["symbol"]),Direction(p["direction"]),float(p["volume"]),None if p["price"] is None else float(p["price"]),None if p["sl"] is None else float(p["sl"]),None if p["tp"] is None else float(p["tp"]),IntentState(p["state"]),datetime.fromisoformat(p["created_at"]),int(p["send_count"]),None if p.get("broker_ticket") is None else int(p["broker_ticket"]),None if p.get("position_ticket") is None else int(p["position_ticket"]),str(p.get("reason","")))
