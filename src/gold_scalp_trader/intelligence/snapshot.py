...
from .confluence import ConfluenceReport
...
class TimeframeIntelligence:
    indicators: IndicatorSeries; quant: QuantReport; structure: StructureReport; technical: TechnicalReport; liquidity: LiquidityReport; confluence: ConfluenceReport
...
class IntelligenceSnapshot:
    ...
    def __post_init__(self) -> None:
        object.__setattr__(self,"by_timeframe",MappingProxyType(dict(self.by_timeframe)))
...
def build(market: MarketSnapshot, news: NewsContext | None=None) -> IntelligenceSnapshot:
    ...
    for tf,candles in market.candles.items():
        ...
        series,quant=calculate_quant(candles); structure=analyze_structure(candles); technical=analyze_technical(structure,quant,price); liquidity=analyze_liquidity(candles,structure,quant)
        reports[tf]=TimeframeIntelligence(series,quant,structure,technical,liquidity,ConfluenceReport())
