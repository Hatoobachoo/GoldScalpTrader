...
Only this module imports MetaTrader5 for analytical reads. Irreversible broker
operations are intentionally absent and belong exclusively to execution/mt5_writer.py.
"""
...
class Mt5Reader:
    ...
    def read(self, *, captured_at: datetime | None = None) -> ReadResult:
        ...
        snapshot = build_snapshot(captured_at=captured, account=account, symbol_spec=spec, quote=quote,
                                  candles=candles, quality=quality, positions=positions,
    ...
    def _read_account(self) -> AccountFacts:
        ...
        try:
            return AccountFacts(int(_field(raw, "login")), str(_field(raw, "server")), str(_field(raw, "currency")),
    ...
    def _read_symbol_spec(self, symbol: str) -> SymbolSpec:
        ...
        try:
            return SymbolSpec(symbol=symbol, digits=int(_field(raw, "digits")), point=float(_field(raw, "point")),
                              ...
                              trade_mode=_field(raw, "trade_mode", None), filling_mode=_field(raw, "filling_mode", None))
        except (TypeError, ValueError) as exc:
    ...
    def _read_candles(self, symbol: str, captured: datetime) -> tuple[dict[Timeframe, tuple[Candle, ...]], dict[Timeframe, DataQuality]]:
        counts = {Timeframe.M1: self.settings.m1_history_bars, Timeframe.M5: self.settings.m5_history_bars,
                  Timeframe.M15: self.settings.m15_history_bars, Timeframe.H1: self.settings.h1_history_bars,
        ...
        for timeframe, count in counts.items():
            ...
            try:
                for row in raw:
                    candle = Candle(timeframe, _utc_from_epoch(_field(row, "time")), float(_field(row, "open")),
                                    float(_field(row, "high")), float(_field(row, "low")), float(_field(row, "close")),
                                    int(_field(row, "tick_volume", 0) or 0), int(_field(row, "real_volume", 0) or 0))
    ...
    def _read_positions(self, symbol: str) -> tuple[tuple[PositionFacts, ...] | None, DataQuality]:
        ...
        for item in raw:
            ...
            result.append(PositionFacts(int(_field(item, "ticket")), str(_field(item, "symbol")), direction,
                                        float(_field(item, "volume")), float(_field(item, "price_open")),
                                        None if sl == 0.0 else sl, None if tp == 0.0 else tp,
                                        _optional_int(_field(item, "magic", None)), str(_field(item, "comment", "") or "")))
