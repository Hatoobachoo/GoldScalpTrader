...
Only this module imports MetaTrader5 for analytical/recovery reads. Irreversible
broker operations are intentionally absent and belong exclusively to
...
class Mt5Reader:
    ...
    def read_position_deals(
        self,
        position_ticket: int,
        *,
        from_time: datetime,
        to_time: datetime,
    ) -> tuple[DealFacts, ...] | None:
        """Read normalized deal lineage for one known broker position.

        `None` means the broker history read was unavailable. An empty tuple is a
        positively successful read with no matching deals in the requested range.
        """
        ...
        try:
            for item in raw:
                ...
                if position_id is not None and position_id != position_ticket:
                    continue
                type_value = _field(item, "type", None)
                deal_type, direction = _deal_type(self.api, type_value)
                ...
                deals.append(
                    DealFacts(
                        ...
                        entry_role=_deal_entry_role(self.api, _field(item, "entry", None)),
                        deal_type=deal_type,
                        time_utc=time_utc,
    ...
    def _read_account(self) -> AccountFacts:
        ...
        try:
            return AccountFacts(
                int(_field(raw, "login")),
                str(_field(raw, "server")),
                str(_field(raw, "currency")),
                float(_field(raw, "balance")),
                float(_field(raw, "equity")),
                float(_field(raw, "margin_free")),
                _optional_bool(_field(raw, "trade_allowed")),
                _optional_bool(_field(raw, "trade_expert")),
            )
        except (TypeError, ValueError) as exc:
    ...
    def _read_symbol_spec(self, symbol: str) -> SymbolSpec:
        ...
        try:
            return SymbolSpec(
                symbol=symbol,
                digits=int(_field(raw, "digits")),
                point=float(_field(raw, "point")),
                tick_size=float(_field(raw, "trade_tick_size", _field(raw, "point"))),
                tick_value=None if tick_value in (None, 0) else float(tick_value),
                volume_min=float(_field(raw, "volume_min")),
                volume_max=float(_field(raw, "volume_max")),
                volume_step=float(_field(raw, "volume_step")),
                stops_level_points=int(_field(raw, "trade_stops_level", 0) or 0),
                freeze_level_points=int(_field(raw, "trade_freeze_level", 0) or 0),
                trade_mode=_field(raw, "trade_mode", None),
    ...
    def _read_quote(self, symbol: str, captured: datetime) -> Quote:
        ...
        source_time = (
            datetime.fromtimestamp(float(source_msc) / 1000.0, tz=UTC)
            if source_msc is not None
            else _utc_from_epoch(_field(raw, "time"))
        )
        try:
    ...
    def _read_candles(
        ...
        symbol: str,
        captured: datetime,
    ) -> tuple[dict[Timeframe, tuple[Candle, ...]], dict[Timeframe, DataQuality]]:
        counts = {
            Timeframe.M1: self.settings.m1_history_bars,
            Timeframe.M5: self.settings.m5_history_bars,
            Timeframe.M15: self.settings.m15_history_bars,
            Timeframe.H1: self.settings.h1_history_bars,
            Timeframe.H4: self.settings.h4_history_bars,
        }
        ...
        for timeframe, count in counts.items():
            ...
            if raw is None:
                output[timeframe] = ()
                quality[timeframe] = DataQuality.UNKNOWN
                continue
            candles: list[Candle] = []
            try:
                for row in raw:
                    candle = Candle(
                        timeframe,
                        _utc_from_epoch(_field(row, "time")),
                        float(_field(row, "open")),
                        float(_field(row, "high")),
                        float(_field(row, "low")),
                        float(_field(row, "close")),
                        int(_field(row, "tick_volume", 0) or 0),
                        int(_field(row, "real_volume", 0) or 0),
                    )
                    if candle.close_time <= captured:
                ...
                raise Mt5ReadError(f"{timeframe.value} candle data corrupt") from exc
            output[timeframe] = tuple(candles)
            quality[timeframe] = DataQuality.HEALTHY if len(candles) >= min(count, 50) else DataQuality.INSUFFICIENT
    ...
    def _read_positions(self, symbol: str) -> tuple[tuple[PositionFacts, ...] | None, DataQuality]:
        ...
        result: list[PositionFacts] = []
        for item in raw:
            type_value = int(_field(item, "type", -1))
            direction = Direction.BUY if type_value == 0 else Direction.SELL if type_value == 1 else Direction.NONE
            if direction is Direction.NONE:
                raise Mt5ReadError("position direction is unknown")
            sl = float(_field(item, "sl", 0.0) or 0.0)
            tp = float(_field(item, "tp", 0.0) or 0.0)
            result.append(
                PositionFacts(
                    int(_field(item, "ticket")),
                    str(_field(item, "symbol")),
                    direction,
                    float(_field(item, "volume")),
                    float(_field(item, "price_open")),
                    None if sl == 0.0 else sl,
                    None if tp == 0.0 else tp,
                    _optional_int(_field(item, "magic", None)),
                    str(_field(item, "comment", "") or ""),
                )
            )
